...
def adjust_stock():
    ...
    print(f"Received data: {data}")  # Debugging: Überprüfen der eingehenden Daten
    shop_id = data.get('shop_id')
    item_id = data.get('item_id')
    adjustment = data.get('adjustment', 0)

    # Überprüfen, ob die Parameter korrekt übergeben wurden
    if not all([shop_id, item_id, adjustment]):
        return jsonify({'error': 'Ungültige Anfrageparameter'}), 400

    # Bestandsdatensatz suchen oder erstellen
    ...
    print(f"Updated stock: {stock.quantity}")  # Debugging: Überprüfen des aktualisierten Bestands
...
def delete_shop(shop_id):
    shop = Shop.query.get_or_404(shop_id)
    if shop.stocks:  # Sicherstellen, dass keine Bestände mehr vorhanden sind
        return jsonify({"error": "Shop kann nicht gelöscht werden, da er noch Bestände enthält."}), 400
...
# Exportfunktion als Excel-Datei
